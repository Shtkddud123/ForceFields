...
{ 
  ...
  /*                     c (CH2)  */
  /*--------------------------------*/
  /*This code is for the Au-OA bead */
  /*--------------------------------*/
  ...
  bondlength = 1.40; /*This 'bondlength' is just for between CH2-O*/ 
  bondangle = 106*3.14159/180; /*bond angle taken from parameter files.*/ 
  bx = 0.0;
  by = 0.0; 
  bz = 2.244; /*taken from the sigma value of O*/  
  ...
  dx = -1*0.96*sin(bondangle/2); /*Here, the bond length of H-O is the bondlength*/ 
  dy = 0.0; 
  dz = bz + 0.96*cos(bondangle/2);
  /* printf("%lf %lf %lf \n", dx, dy, dz); */
  
  /* Defining the triad LJ parameters*/ 
  /* Units should be changed to angstroms and kJ mol^-1 for sigma and epsilon respectively*/ 
  
  bepsilon = 0.1921; /*Taken from CHARMM parameter files */
  bsigma = 3.145; /*Taken from CHARMM parameter files*/

  cepsilon = 0.1004; /*Taken from klein et al, 2008*/
  csigma = 4.0500; /*Taken from kelin et al, 2008*/  

  depsilon = 0.046; /*Taken from CHARMM parameter files */ 
  dsigma = 0.4000; /*Taken from CHARMM parameter files*/ 
  ...
  cmass = 1.0;   /* We are assuming here that the c atom is H*/ 
