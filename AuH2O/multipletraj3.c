...
/*Failed attempt at fixing the PDB!!*/
...
{ 
  ...
  /*LJ parameters*/ 
  double Oepsilon = 0.1921;
  double Osigma = 3.1448;
  ...
  double V_ave2[nbins]; /*The average interaction energy*/ 
  double sumofweight2[nbins]; /*sum of weights*/ 
  double weights2[nbins];  
  int bin_counter2[nbins]; 
  double newLJ2[nbins];


  /*Output file for testing COM*/ 

  //  int atomcounter; 

  //  FILE *testfile; 

  //  testfile = fopen("COM.gro", "w"); 
...
//  if(testfile == NULL) {
  ...
  for(trajno=0;trajno<numberoftraj;trajno++) {
    ...
    //printf("\n");
    // printf("This is the data for frame no %d \n", trajno); 
    ...
    /*Accounting for the periodic boundary condition when calculating the position for each atom in a water molecule*/
	...
	//	fprintf(testfile,"%5i sol O    %5i%8.3f%8.3f%8.3f \n", i+1, atomcounter+1, Obcoordinatex/10,Obcoordinatey/10,Obcoordinatez/10); 
	//	atomcounter++; 

	//	fprintf(testfile,"%5i sol O    %5i%8.3f%8.3f%8.3f \n", i+1, atomcounter+1, Occoordinatex/10,Occoordinatey/10,Occoordinatez/10); 
	//	atomcounter++; 

	//	fprintf(testfile,"%5i sol O    %5i%8.3f%8.3f%8.3f \n", i+1, atomcounter+1, Odcoordinatex/10,Odcoordinatey/10,Odcoordinatez/10); 
	//	atomcounter++; 

	//	fprintf(testfile,"%5i COM S    %5i%8.3f%8.3f%8.3f \n", i+1, atomcounter+1, xmass/10,ymass/10,zmass/10); 
	//	atomcounter++; 
	...
	weight2 = (1/(double)watercounter[w1[i]] + 1/(double)watercounter[w2[i]] + 1/(double)watercounter[w3[i]]);
	...
	for (j=0;j<nbins;j++) { 
	  ...
	  if (COMdistgoldwater > bin_min[j] && COMdistgoldwater <= bin_max[j]) {  
	    ...
	    V_ave2[j] +=boxLJ*weight2;  
	    sumofweight2[j] += weight2;
	    bin_counter2[j]++; 
	    newLJ2[j] = V_ave2[j]/sumofweight2[j];
	       
	  }
	   
	  //  printf("weight %lf %lf %d %lf %lf \n", bin_min[j],V_ave[j],bin_counter[j],newLJ[j],sumofweight[j]); 
	  // printf("weight2 %lf %lf %d %lf %lf \n", bin_min[j],V_ave2[j],bin_counter2[j],newLJ[j],sumofweight2[j]); 
  ...
  for (j=0;j<nbins;j++) {  
    ...
    //   printf("weight2 %lf %lf %d \n", bin_min[j],newLJ2[j],bin_counter2[j]);
		    
  }
  
  //fprintf(testfile,"%12.3f%12.3f%12.3f \n",boxlength[0]/10,boxlength[1]/10,boxlength[2]/10); 
  //fclose(testfile);
  fclose(ipf);
  return(0); 
}
